...
class DataIngestion:
    ...
    """Class for ingesting data into AstraDB Vector Store."""
    
    def __init__(self):
        
        """Initialize DataIngestion with environment variables and configurations."""
    ...
    def transform_data(self):
        ...
        for _, row in self.product_data.iterrows():
            product_entry = {
                    ...
                    "product_title": str(row["product_title"]) if not pd.isna(row["product_title"]) else None,
                    "rating": str(row["rating"]) if not pd.isna(row["rating"]) else None,
                    "total_reviews": str(row["total_reviews"]) if not pd.isna(row["total_reviews"]) else None,
                    "price": str(row["price"]).replace("â‚¹", "₹") if not pd.isna(row["price"]) else None,
    ...
    def store_in_vector_db(self,documents: List[Document]):
        ...
        """
        print("Storing documents into AstraDB vector store...")
